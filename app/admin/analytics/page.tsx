  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <Card className="space-y-3 p-5">
          <h2 className="font-semibold text-navy">Paid volume by month</h2>
          {analytics.byMonth.map((item) => (
            <Bar
              key={item.label}
              label={item.label}
          ...
          <dl className="mt-3 space-y-2 text-sm">
            {analytics.usersByRole.map((item) => (
              <div key={item.role} className="flex justify-between">
                ...
                <dd className="number font-semibold text-navy">{item.count}</dd>
              </div>
            ))}
          </dl>
        </Card>
        <Card className="p-5">
          <h2 className="font-semibold text-navy">Disputes</h2>
          <dl className="mt-3 space-y-2 text-sm">
