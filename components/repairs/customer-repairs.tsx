...
export function CustomerRepairs({
  ...
  return (
    <div className="bg-[#e8eef4] text-navy">
      <Hero />
      <div className="relative z-10 mx-auto -mt-10 max-w-[1180px] px-6 pb-12">
              ...
              {TABS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "-mb-px border-b-2 px-3 py-3 text-sm font-semibold",
...
function Hero() {
  return (
    <section className="relative overflow-hidden pb-20 pt-24">
      ...
      <img src="/landing/repairs-hero.png" alt="" className="absolute inset-0 h-full w-full object-cover object-[62%_center]" />
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(7,20,34,0.88)_0%,rgba(7,20,34,0.42)_52%,rgba(7,20,34,0.12)_100%)]" />
      <p className="font-script absolute right-[5%] top-24 hidden max-w-[170px] text-right text-[28px] leading-tight text-white lg:block">
        Less Time
        <br />
        Waiting.
        <br />
...
function RepairItem({ row }: { row: RepairRow }) {
  return (
    <article className="grid gap-4 py-5 md:grid-cols-[92px_minmax(0,0.9fr)_minmax(0,1.15fr)_110px_128px] md:items-center">
      <div className="h-[72px] overflow-hidden rounded-xl">
        {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      <div className="min-w-0">
        ...
        <div className="mt-2 flex items-center gap-2">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={row.shopPhoto} alt="" className="h-7 w-7 rounded-full object-cover" />
        ...
        return (
          <li key={step} className="flex min-w-0 flex-1 flex-col items-center">
            <div className="flex w-full items-center">
              <span className={cn("h-0.5 flex-1", index === 0 ? "bg-transparent" : done || active ? "bg-[#2f7bff]" : "bg-line")} />
              <span
                className={cn(
                  "rounded-full",
                  active ? "h-3 w-3 bg-[#2f7bff] ring-4 ring-[#2f7bff]/20" : done ? "h-2 w-2 bg-[#2f7bff]" : "h-2 w-2 border border-[#c5d0dc] bg-white",
                )}
              />
              <span className={cn("h-0.5 flex-1", index === steps.length - 1 ? "bg-transparent" : done ? "bg-[#2f7bff]" : "bg-line")} />
            </div>
            <span className="mt-1 w-[4.6rem] text-center text-[9px] leading-tight text-muted">{step}</span>
          </li>
        );
      })}
    </ol>
